                    ...
                    .enqueue(object : retrofit2.Callback<List<Employee>>{
                        override fun onResponse(call: Call<List<Employee>>, response: Response<List<Employee>>){
                            response.body()?.forEach{
                                employeeItemsList.add(Employee(it.emp_id,it.emp_name,it.emp_gender,it.emp_email,it.emp_salary))
                            }
                        }
    ...
    Column {
        ...
        Row(modifier = Modifier
            ...
            Column(modifier = Modifier.weight(0.85f)) {
                Text(text = "Employee List: ${employeeItemsList.size}", fontSize = 25.sp)
            }
            Button(onClick = {
        ...
        LazyColumn(verticalArrangement = Arrangement.spacedBy(6.dp)){
            ...
            itemsIndexed(items=employeeItemsList){
                    index, item ->
                Card(modifier = Modifier
                    ...
                    Row(modifier = Modifier
                        ...
                        .height(Dp(130f))
                        .padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
                        Text(text = "Name: ${item.emp_name}\n"+
                                "Gender: ${item.emp_gender}\n"+
                                "Email: ${item.emp_email}\n"+
                            "Salary: ${item.emp_salary}", Modifier.weight(0.85f))
                        TextButton(onClick = {
                            itemClick = item
                            navController.currentBackStackEntry?.savedStateHandle?.set("data",
                                Employee(item.emp_id,item.emp_name,item.emp_gender,item.emp_email,item.emp_salary))
